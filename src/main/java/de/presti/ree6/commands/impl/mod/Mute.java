        ...
        if (commandEvent.getMember().hasPermission(Permission.MODERATE_MEMBERS)) {
            ...
            if (commandEvent.isSlashCommand()) {
                ...
                if (targetOption != null && timeOption != null && targetOption.getAsMember() != null) {
                    ...
                    Report.sendModWebhook(commandEvent, Objects.requireNonNull(targetOption.getAsMember()), 1, reasonOption != null ? reasonOption.getAsString() : "", "По требованию", commandEvent.getMember());
                } else {
                ...
                if (commandEvent.getArguments().length <= 2 && commandEvent.getArguments().length <= 3) {
                    if (commandEvent.getMessage().getMentions().getMembers().isEmpty()) {
                        ...
                        Report.sendModWebhook(commandEvent, commandEvent.getMessage().getMentions().getMembers().get(0), 1, reason  , "По требованию", commandEvent.getMember());
                    }
                } else {
